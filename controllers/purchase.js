...
exports.premium = async(req, res, next) => {
    ...
    // });
    try {
        var instance = new Razorpay({ key_id: 'rzp_test_0vMGyXdywrzGmu', key_secret: 'SiqSj5HJZY5R5Iprki6Ha6x6' });
        ...
        const ordertable =await Order.create({orderId: order.id, status: 'PENDING',expenseuserid:req.user.userid});
...
exports.transactionstatus = async(req, res, next) => {
    ...
    try{
        ...
        const promise2 = 
            ...
            await User.update({isPremiumUser: false},{
                where: {
                  id:userId
                }}) ;
